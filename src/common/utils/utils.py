...
# def get_commodity_time(df: pd.DataFrame, fr: np.int32, to: np.int32) -> np.int32:
#     return (df[(df.fromnode == fr) & (df.tonode == to)].c)


# def get_time_matrix(df: pd.DataFrame, n: np.int32) -> np.ndarray:
#     time_matrix = np.empty([n, n])
#     for i in np.arange(n):
#         for j in np.arange(n):
#             time_matrix[i, j] = get_commodity_time(df, i, j)
#     return (time_matrix)


# def get_commodity_demand(df: pd.DataFrame, fr: np.int32, to: np.int32) -> np.float32:
#     return (df[(df.fromnode == fr) & (df.tonode == to)].w)


# def get_demand_matrix(df: pd.DataFrame, n: np.int32) -> np.ndarray:
#     demand_matrix = np.empty([n, n])
#     for i in np.arange(n):
#         for j in np.arange(n):
#             demand_matrix[i, j] = get_commodity_demand(df, i, j)
#     return (demand_matrix)

# def write_results_from_paths(results: list, n, p, alpha, perc, r, reported_time, suffix=''):
#     makedirs('../paths', exist_ok=True)
#     file_name = f'../paths/data_{n}_{p}_{alpha}_{perc}_0.1_{r}_3{suffix}.csv'
#     all_edges_no_diag = [(i, j) for i in range(n) for j in range(n) if i != j]
